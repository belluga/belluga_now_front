...
void main() {
  ...
  test(
    'invite contact import cache can read legacy single-key payload',
    () async {
      final storage = _FakeInviteContactImportCacheStorage();
      final cache = InviteContactImportCache(storage: storage);
  ...
  test(
    'invite contact import cache erases all persisted contact projections',
    () async {
      final storage = _FakeInviteContactImportCacheStorage();
      final cache = InviteContactImportCache(storage: storage);
      final entry = InviteContactImportCacheEntry(
      ...
      await cache.write('former-user', entry);
      await storage.write(
        'invite_contact_import_cache_v1:legacy-former-user',
        '{"signature":"legacy"}',
      );
