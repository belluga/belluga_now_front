...
void main() {
  ...
  testWidgets('Location section shows map CTA copy', (tester) async {
    ...
    await tester.pumpWidget(MaterialApp(home: LocationSection(event: event)));
  ...
  testWidgets('Venue card shows profile button', (tester) async {
    final venue = AccountProfileSummary(
      idValue: AccountProfileTextValue(
        MockScheduleBackend.generateMongoId('test-venue'),
      ),
      nameValue: AccountProfileNameValue()..parse('Test Venue'),
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(body: VenueCard(venue: venue)),
      ),
    );
  ...
  testWidgets(
    'Venue card pushes partner detail route with the canonical slug',
    (tester) async {
      final venue = AccountProfileSummary(
        idValue: AccountProfileTextValue(
          MockScheduleBackend.generateMongoId('test-venue-route'),
        ),
        nameValue: AccountProfileNameValue()..parse('Venue Navegavel'),
        slugValue: null,
        profileTypeValue: AccountProfileTypeValue('venue'),
        canOpenPublicDetailValue: _boolValue(true),
        publicDetailPathValue: AccountProfilePublicDetailPathValue(
      ...
      final router = RootStackRouter.build(
        routes: [
          ...
          NamedRouteDef(
            ...
            path: '/parceiro/:slug',
            builder: (_, data) =>
                Scaffold(body: Text('detail:${data.params.getString('slug')}')),
          ),
        ],
  ...
  testWidgets(
    'Venue card hides profile button when public detail is disabled',
    (tester) async {
      final venue = AccountProfileSummary(
        idValue: AccountProfileTextValue(
          MockScheduleBackend.generateMongoId('test-venue-hidden'),
        ),
        nameValue: AccountProfileNameValue()..parse('Venue Fechado'),
  ...
  testWidgets(
    ...
    (tester) async {
      final venue = AccountProfileSummary(
        idValue: AccountProfileTextValue(
          MockScheduleBackend.generateMongoId('test-venue-empty-path'),
        ),
        nameValue: AccountProfileNameValue()..parse('Venue Sem Path'),
