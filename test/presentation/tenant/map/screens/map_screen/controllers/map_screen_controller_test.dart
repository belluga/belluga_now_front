...
void main() {
  ...
  group('MapScreenController telemetry', () {
    ...
    test(
      'keeps last selected poi memory when closing the card explicitly',
      () async {
    ...
    test(
      'pan gesture on map collapses expanded filters back to discovery',
      () async {
        final fakeMapHandle = _FakeMapHandle();
        final localController = _buildMapController(
          poiRepository: _buildPoiRepository(mapRepository: mapRepository),
    ...
    test(
      'typing search text debounces requests and uses the latest term',
      () async {
    ...
    test(
      'search tray captures current map center and preserves it on clear',
      () async {
        final fakeMapHandle = _FakeMapHandle(
    ...
    test(
      'catalog filter apply does not emit global loading status message',
      () async {
        ...
        final subscription = controller.statusMessageStreamValue.stream.listen(
          statusMessages.add,
        );
    ...
    test(
      'filter clear keeps pending chip label without global loading message',
      () async {
        controller.toggleCatalogCategoryFilter(
          _buildCategory(key: 'restaurant', label: 'Restaurantes'),
        );
        await _flushMicrotasks();
    ...
    test('applies dynamic category filters using category keys', () async {
      controller.toggleCatalogCategoryFilter(
        _buildCategory(key: 'nature', label: 'Natureza', tags: const {}),
      );
      await _flushMicrotasks();
    ...
    test(
      ...
      () async {
        ...
        expect(fakeMapHandle.lastVerticalViewportAnchor, closeTo(0.40, 1e-9));
      },
    );
    ...
    test(
      ...
      () async {
        ...
        localController.toggleCatalogCategoryFilter(
          _buildCategory(key: 'event', label: 'Eventos', tags: const {}),
        );
        await _flushMicrotasks();
        await _flushMicrotasks();
    ...
    test(
      ...
      () async {
        ...
        final ordered = controller.orderedFilterResultPois(<CityPoiModel>[
          _buildPoi(
            id: 'poi-past',
            name: 'Evento passado',
            refType: 'event',
    ...
    test(
      'tapping the same active catalog filter reopens filter results tray',
      () async {
        final category = _buildCategory(key: 'events', label: 'Eventos');
    ...
    test(
      ...
      () async {
        ...
        expect(
          _querySource(mapRepository.lastQuery),
    ...
    test('keeps stale map data visible when loadPois fails', () async {
      mapRepository.nextPois = <CityPoiModel>[_buildPoi(id: 'poi-a')];
    ...
    test(
      'locks filter interactions while a filter reload is in flight',
      () async {
        final firstRequest = Completer<List<CityPoiModel>>();
        mapRepository.queuedFetchCompleters.add(firstRequest);
        ...
        expect(controller.filterInteractionLockedStreamValue.value, isFalse);
    ...
    test(
      'ignores marker taps immediately after a filter reload settles',
      () async {
        final fakeMapHandle = _FakeMapHandle();
        final localController = _buildMapController(
          poiRepository: _buildPoiRepository(mapRepository: mapRepository),
    ...
    test(
      'accepts marker taps after post-filter suppression window expires',
      () async {
        final fakeMapHandle = _FakeMapHandle();
        final localController = _buildMapController(
          poiRepository: _buildPoiRepository(mapRepository: mapRepository),
    ...
    test(
      ...
      () async {
        ...
        expect(localController.selectedPoiStreamValue.value, isNull);
        expect(
          localAccountProfilesRepository.getAccountProfileBySlugCallCount,
          1,
        );
    ...
    test(
      'cluster marker tap opens picker dock instead of detailed card',
      () async {
        final fakeMapHandle = _FakeMapHandle();
        final localController = _buildMapController(
          poiRepository: _buildPoiRepository(mapRepository: mapRepository),
    ...
    test(
      'cluster picker selection hydrates chosen poi like direct selection',
      () async {
        final localAccountProfilesRepository = _FakeAccountProfilesRepository();
        final fakeMapHandle = _FakeMapHandle();
        final localController = _buildMapController(
        ...
        localController.showClusterPicker(<CityPoiModel>[
          poiA,
          poiB,
        ...
        expect(
          localController.selectedPoiLoadingIdStreamValue.value,
          'poi-stack-b',
        );
        expect(localController.selectedPoiStreamValue.value, isNull);
    ...
    test(
      'deck poi selection hydrates account profile with avatar and cover',
      () async {
        final localAccountProfilesRepository = _FakeAccountProfilesRepository();
        final fakeMapHandle = _FakeMapHandle();
        final localController = _buildMapController(
    ...
    test(
      ...
      () async {
        ...
        expect(
          localController.selectedPoiStreamValue.value?.linkedProfiles.map(
    ...
    test(
      'clearing selection before hydration completes ignores stale result',
      () async {
        final localAccountProfilesRepository = _FakeAccountProfilesRepository();
        final fakeMapHandle = _FakeMapHandle();
        final localController = _buildMapController(
    ...
    test(
      'skips auto-centering on user when initial poi query is provided',
      () async {
        final targetPoi = _buildPoi(
          id: 'poi-target',
          refType: 'event',
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          MapScreenController.missingDefaultOriginConfigurationMessage,
        );
        expect(
    ...
    test(
      'recenters once to the live user origin after the first bootstrap when the map becomes ready',
      () async {
        final fakeMapHandle = _FakeMapHandle(isReady: false);
        final tenantDefaultOrigin = _buildCoordinate('-20.4000', '-40.4000');
        final localController = _buildMapController(
          poiRepository: _buildPoiRepository(mapRepository: mapRepository),
          userLocationRepository: userLocationRepository,
          telemetryRepository: telemetry,
          mapHandle: fakeMapHandle,
          appData: _buildAppData(defaultOrigin: tenantDefaultOrigin),
        );
        final refreshCompleter = Completer<bool>();
        userLocationRepository.refreshIfPermittedCompleter = refreshCompleter;
        addTearDown(() async {
    ...
    test(
      ...
      () async {
        ...
        final statusSubscription = controller.statusMessageStreamValue.stream
            .listen(statusMessages.add);
        ...
        final firstResolvedCoordinate = _buildCoordinate(
          '-20.1234',
          '-40.2345',
        );
        userLocationRepository.userLocationStreamValue.addValue(
          firstResolvedCoordinate,
        );
        userLocationRepository.lastKnownLocationStreamValue.addValue(
          firstResolvedCoordinate,
        );
    ...
    test('centerOnUser silently no-ops when map is not ready yet', () async {
      ...
      userLocationRepository.userLocationStreamValue.addValue(
        _buildCoordinate('-20.1000', '-40.1000'),
      );
    ...
    test(
      ...
      () async {
        ...
        mapRepository.nextStackItems = <CityPoiModel>[topPoi, stackedTarget];
    ...
    test(
      ...
      () async {
        mapRepository.nextPois = <CityPoiModel>[
          _buildPoi(id: 'poi-other', refType: 'event', refId: 'evt-other'),
        ];
    ...
    test(
      'sets deterministic status when initial poi query cannot be resolved',
      () async {
        mapRepository.nextPois = <CityPoiModel>[
          _buildPoi(
            id: 'poi-other',
    ...
    test(
      'handleMarkerTap focuses selected poi in the upper viewport safe zone',
      () async {
        final fakeMapHandle = _FakeMapHandle(
          initialZoom: 15.4,
          initialCenter: _buildPoi(id: 'seed').coordinate,
  ...
  group('MapScreen safe back', () {
    ...
    testWidgets(
      'map screen shows filter-first bottom dock and discovery tray',
      (tester) async {
        final router = _RecordingStackRouter()..canPopResult = false;
        ...
        controller.filterOptionsStreamValue.addValue(
          PoiFilterOptions(
            categories: <PoiFilterCategory>[
              ...
              _buildCategory(key: 'praia', label: 'Praia'),
            ],
          ),
        );
        await tester.pump();
    ...
    testWidgets(
      'overflowing filter cluster exposes handle and expands on tap',
      (tester) async {
        final router = _RecordingStackRouter()..canPopResult = false;
        ...
        expect(
          find.byKey(
            const ValueKey<String>('map-filter-cluster-wrap-expanded'),
          ),
          findsOneWidget,
        );
    ...
    testWidgets('status banner renders above the dock instead of over it', (
      ...
      final trayFinder = find.byKey(
        const ValueKey<String>('map-tray-surface-discovery'),
      );
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        controller.showClusterPicker(<CityPoiModel>[
    ...
    testWidgets('search tray shows Nessa área with a scrollable result list', (
      ...
      controller.filteredPoisStreamValue.addValue(
        List<CityPoiModel>.generate(
          6,
          (index) =>
              _buildPoi(id: 'poi-search-$index', name: 'Resultado $index'),
        ),
      );
      await tester.pump();
      ...
      (tester) async {
        ...
        controller.filteredPoisStreamValue.addValue(<CityPoiModel>[
          _buildPoi(
            id: 'poi-far',
            name: 'Mais longe',
          ).copyWith(distanceMetersValue: _buildDistanceValue(900)),
          _buildPoi(
            id: 'poi-near',
            name: 'Mais perto',
          ).copyWith(distanceMetersValue: _buildDistanceValue(120)),
        ]);
        await tester.pump();
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        controller.mapTrayModeStreamValue.addValue(MapTrayMode.filterResults);
        controller.activeCategoryKeysStreamValue.addValue(const <String>{
          'event',
        });
        controller.filterOptionsStreamValue.addValue(
          PoiFilterOptions(
    ...
    testWidgets('selected filter state exposes an explicit clear affordance', (
      ...
      expect(controller.activeFilterLabelStreamValue.value, isNull);
      expect(
        find.byKey(const ValueKey<String>('map-filter-cluster-wrap-collapsed')),
        findsOneWidget,
      );
    });
      ...
      (tester) async {
        ...
        controller.filterOptionsStreamValue.addValue(
          PoiFilterOptions(
            categories: <PoiFilterCategory>[
              _buildCategory(key: 'events', label: 'Eventos'),
              _buildCategory(key: 'praia', label: 'Praia'),
              _buildCategory(key: 'food', label: 'Gastronomia'),
            ],
          ),
        );
        ...
        final positions = <String, Offset>{
          'events': tester.getTopLeft(
            find.byKey(
              const ValueKey<String>('map-compact-filter-chip-events'),
            ),
          ),
    ...
    testWidgets('filter tray renders only backend catalog entries', (
      ...
      controller.filterOptionsStreamValue.addValue(
        PoiFilterOptions(
          categories: <PoiFilterCategory>[
            _buildCategory(key: 'curated_backend', label: 'Filtro Curado'),
          ],
        ),
      );
      controller.filteredPoisStreamValue.addValue(<CityPoiModel>[
    ...
    testWidgets('event cards keep the same CTA labels as other pois', (
      ...
      await _pumpPoiDetailDeck(tester, controller: controller, router: router);
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final firstPoi = _buildPoi(
          ...
          name: 'Praia das Virtudes',
          coverImageUri:
              'https://tenant.test/media/praia-das-virtudes-cover.png',
        );
        final secondPoi = _buildPoi(
          id: 'poi-image-b',
          name: 'Praia das Castanheiras',
...
AppData _buildAppData({
  ...
  if (includeDefaultOrigin) {
    mapUi['default_origin'] = <String, dynamic>{
      'lat': resolvedDefaultOrigin.latitude,
      'lng': resolvedDefaultOrigin.longitude,
      'label': 'Centro',
    };
  }
  final remoteData = {
