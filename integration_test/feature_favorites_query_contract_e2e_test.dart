...
void main() {
  ...
  testWidgets(
    ...
    (_) async {
      ...
      final accountProfiles = await backend.accountProfiles
          .fetchAccountProfilesPage(page: 1, pageSize: 10);
      ...
      expect(
        ...
        isFalse,
        reason:
            'Unfavorited account profile must disappear from GET /favorites.',
      );
    },
  );
}
...
Future<List<FavoritePreviewDTO>> waitForFavoriteState(
  ...
  while (DateTime.now().isBefore(deadline)) {
    ...
    final containsTarget = latest.any(
      (item) => isFavoriteTarget(item, targetId),
    );
...
void assertFavoritesOrdering(List<FavoritePreviewDTO> items) {
  ...
  for (var index = 1; index < items.length; index++) {
    ...
    expect(
      ...
      isTrue,
      reason:
          'Invalid bucket ordering at index $index '
    ...
    if (previousBucket == 1) {
      ...
      expect(
        ...
        isTrue,
        reason:
            'next_event_occurrence_at must be ascending inside bucket 1 at '
            'index $index.',
      );
      continue;
    }
...
int sortBucket(FavoritePreviewDTO item) {
  if (item.liveNowEventOccurrenceId != null) {
    return 0;
  }
